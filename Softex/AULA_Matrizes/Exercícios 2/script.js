...
// Cabeçalhos da média e situação
tabelaHTML += "<th>Média</th><th>Situação</th></tr></thead><tbody>";
...
for (let i = 0; i < alunos.length; i++) {
  ...
  for (let j = 0; j < aluno.notas.length; j++) {
    ...
    let classeSituacao = media >= 7 ? "aprovado" : "reprovado";

    tabelaHTML += `<td><strong>${media}</strong></td><td class="${classeSituacao}">${situacao}</td></tr>`;
  } 
